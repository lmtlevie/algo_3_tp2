...
def main(input,input_2, salida, guia):
    df = pd.read_csv(input, sep="\t", index_col="n")
    ax = plt.subplot()

    df_2 = pd.read_csv(input_2, sep="\t", index_col="n")

    ax.plot(df,color='blue',label="DSU naive")
    ax.plot(df_2,color='red',label="DSU union-by-rank")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i_2", "--input_2", required=True)
