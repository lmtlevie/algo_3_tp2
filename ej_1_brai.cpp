...
double probabilidad_de_perder(vector<set<int>> &vecinos,int n){
    ...
    double probabilidad_de_ganar = 0;
    ...
    vector<bool> nodos_procesados(n+1,false);
    for(int i=1;i <= n;i++){
        if(!visitados[i]){
          //  cout << "Componente Conexa" << endl;
            ...
            int nodos_componente = 0;
            int aristas_no_puentes = 0;
            for(auto x: back_edges){
                if( !(aristas_procesadas[x.first][x.second] ||  aristas_procesadas[x.second][x.first]) ) {
                  ...
                  aristas_en_ciclo++;
                  aristas_no_puentes++;
                  if( !nodos_procesados[x.first] ){ nodos_procesados[x.first] = true; nodos_componente++;}
                }
                ...
                while(v != u){
                    ...
                    //cout << v << endl;
                    if(!nodos_procesados[v]){ nodos_componente++;}
                    aristas_en_ciclo++;
                    aristas_no_puentes++;
            ...
            probabilidad_de_ganar += ( ((double)nodos_componente*( (double) nodos_componente-1) ) / 2 ) / (double) formas_de_jugar;
