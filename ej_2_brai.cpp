...
void dfs_3(vector<vector<int>>& vecinos, int v,vector<bool> &visitado, vector<int>& previo){
    ...
    for(int u: vecinos[v]){  
        ...
        if(!visitado[u]){
            ...
            dfs_3(vecinos,u,visitado,previo);
        }
    }
...
void conjuntOptimo(){
    ...
    // ahora que sé quienes son las componentes fuertemente conexas voy a decir en un arreglo quien es el representante de c/nodo.
    vector<int> representantes(n+1,-1);
    ...
    /*
        for(int i = 1 ; i <= n ; i++){
        cout << " el representate de " << i  << " es : " << representantes[i]  ; 
    } 
    */
     ...
     for(auto cfc : componentes){
         ...
         for( int v : cfc.first ){  
             for( int u : D[v] ){  // para cada u en el vecindario de salida de v en D
                if( representantes[u] != representante){
                   D_2[representante].push_back(representantes[u]);
                }
             }
             if( v != representante ){
